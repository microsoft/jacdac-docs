(self["webpackChunkjacdac_docs"] = self["webpackChunkjacdac_docs"] || []).push([[7109],{
...
// EXPORTS
__webpack_require__.d(__webpack_exports__, {
  "default": function() { return /* binding */ DrawerSearchResults; }
});
...
var elasticlunr = __webpack_require__(88453);
;// CONCATENATED MODULE: ./src/components/useSearchIndex.ts
function useSearchIndex(){var{siteSearchIndex}=(0,gatsby_browser_entry.useStaticQuery)("2744294623");var index=(0,react.useRef)(elasticlunr.Index.load(siteSearchIndex.index));return index.current;}
;// CONCATENATED MODULE: ./src/components/shell/useDrawerSearchResults.ts
function useDrawerSearchResults(_searchQuery){var _lastResult$current;var index=useSearchIndex();var normalizeUrl=url=>url?"/"+url.replace(/^\//,"").replace(/(\.tsx)?\/?$/,""):url;var[searchQuery]=(0,useDebounce/* default */.Z)(_searchQuery,500);// debounce duplicate search
var lastResult=(0,react.useRef)(undefined);if(((_lastResult$current=lastResult.current)===null||_lastResult$current===void 0?void 0:_lastResult$current.searchQuery)===searchQuery)return lastResult.current.nodes;// spin up search
//console.debug(`search "${searchQuery}"`)
var nodes=undefined;if(searchQuery&&index){var results=index.search(searchQuery,{expand:true});var indexDocs=results.map(_ref=>{var{ref}=_ref;return index.documentStore.getDoc(ref);});nodes=indexDocs.map(id=>({title:id.title,url:normalizeUrl(id.url)}));}// cache result
lastResult.current={searchQuery,nodes};return nodes;}
...
;// CONCATENATED MODULE: ./src/components/shell/DrawerSearchResults.tsx
function DrawerSearchResults(props){var{searchQuery}=props;var results=useDrawerSearchResults(searchQuery);return/*#__PURE__*/react.createElement(List/* default */.Z,{dense:true},!results.length&&/*#__PURE__*/react.createElement(ListItem/* default */.ZP,null,/*#__PURE__*/react.createElement(Alert/* default */.Z,{severity:"info"},"no results found")),results.map(result=>{var _result$url;return/*#__PURE__*/react.createElement(gatsby_theme_material_ui/* Link */.rU,{key:"search"+result.url,to:result.url,"aria-label":result.title},/*#__PURE__*/react.createElement(ListItem/* default */.ZP,null,/*#__PURE__*/react.createElement(ListItemText/* default */.Z,{primaryTypographyProps:{color:"textPrimary"},secondaryTypographyProps:{color:"textSecondary"},"aria-label":result.title,primary:result.title,secondary:(_result$url=result.url)===null||_result$url===void 0?void 0:_result$url.slice(0)})));}));}

/***/ })

}]);
//# sourceMappingURL=7109-a94eeeb291f2d3063417.js.map